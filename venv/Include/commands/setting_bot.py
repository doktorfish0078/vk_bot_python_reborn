...
def settings_session(vk_api, peer_id, user_sender_id):
    ...
    hello_msg = f"Здравствуйте, мы подтвердили, что Вы являетесь администратором беседы - \"{chat_title}\" c id - \"{chat_id}\"\n" \
                ...
                "Для того, чтобы удалить курс, ссылку или мероприятия, вы можете воспользоваться командами:\n" \
                f"👉/удалить курс {chat_id} <порядковый номер курса>\n" \
                f"👉/удалить ссылку {chat_id} <порядковый номер ссылки>\n" \
                f"👉/удалить мероприятие {chat_id} <порядковый номер мероприятия>\n" \
                "Узнать порядковый номер можно посредством команд:\n" \
                f"👉/список курсов {chat_id}\n" \
                f"👉/список ссылок {chat_id}\n" \
                f"👉/список мероприятий {chat_id}\n"
